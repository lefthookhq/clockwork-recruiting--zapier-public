...
const getAttachment = async (z, bundle) => {
	let response = await z.request(bundle.inputData.url);
	return response;
};
...
const addAttachment = async (z, bundle) => {
	...
	let finalResponse = await z.request({
		url:
			'https://api.clockworkrecruiting.com/v1/{{bundle.authData.firm_subdomain}}/people/{{bundle.inputData.person_id}}',
		method: 'POST',
		json: person,
	});
